...
public static class M_Audio
{
    ...
    private static GameObject certainOneShot;
    ...
    public static void PlayOneShotAudio_StopInCertainTime(string _AudioName)
    {
        GameObject soundGameObject = new GameObject("Sound " + _AudioName);
        certainOneShot = soundGameObject;
        AudioSource audioSource = soundGameObject.AddComponent<AudioSource>();
        audioSource.clip = GetAudioClip(false, _AudioName).audioClip;
        audioSource.volume = GetAudioClip(false, _AudioName).volume * globalVolumeOffset;
        audioSource.loop = true;
    ...
    public static void OneShotAudio_StopCertain()
    {
        if (certainOneShot!=null)
        {
            AudioSource audioSource = certainOneShot.GetComponent<AudioSource>();
            DOTween.To(() => audioSource.volume, x => audioSource.volume = x, 0, 0.3f);
            Object.Destroy(certainOneShot, 0.31f);
            certainOneShot = null;
        }
